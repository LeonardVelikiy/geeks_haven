			...
			<div class="head">Geek's Haven</div>
				...
				<div class="contentOffice_admin">	
					...
					if ($_GET['application']or$_GET['hero_aprove']) 
					{
						...
						// добавление героя в таблицу с полноценными героями
						...
						//удаление из заявок после одобрения 
						$heros_aprove_del=$_GET['hero_aprove'];
						$str_del_heros_aprove="DELETE FROM `character_applications` WHERE id_aplication = $heros_aprove_del";
						$run_del_heros_aprove=mysqli_query($connect,$str_del_heros_aprove);

						//удаление из заявок по нажатию УДАЛИТЬ
